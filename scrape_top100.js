  ...
  request({
    ...
    if (!err && res.statusCode == 200) {
      ...
      charts.getConnection(function (err, conn) {
        if (err) throw err;
        var query = conn.query("INSERT INTO top100 (time,name,interpret,position) VALUES ? ON DUPLICATE KEY UPDATE time=VALUES(time)",
        [songs],
        function(err, res) {
          if (err) {
...
const dates = getDatesArray(2012, 1, 2013, 12);
const total = dates.length;
var current = 0;
const URL = "http://www.billboard.com/charts/hot-100/";
